...
from cats.serializers import CatSerializer, AchievementSerializer
...
class CatViewSetTestCase(APITestCase):
        ...
        def test_list_cats(self):
            ...
            Cat.objects.create(name='Барсик', color='#000000', birth_year=2021, owner=self.user)
            Cat.objects.create(name='Рыжик', color='#ff0000', birth_year=2019, owner=self.user)
        ...
        def test_retrieve_cat_detail(self):
                ...
                cat = Cat.objects.create(name='Васька', color='#888888', birth_year=2022, owner=self.user)
        ...
        def test_delete_own_cat(self):
            ...
            cat = Cat.objects.create(name='Пушок', color='#cccccc', birth_year=2021, owner=self.user)
            url = reverse('cat-detail', kwargs={'pk': cat.pk})
...
class AchievementViewSetTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        resp = self.client.post('/api/token/login/', {
            'username': 'achiever',
            'password': 'supersecret123'
        })
